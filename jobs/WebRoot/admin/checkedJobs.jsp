				...
				<dl class="company_center_content">
					...
					<ul class="reset my_jobs">
					  <li><br/><br/>
					    ...
					    <label><input type="radio" name="check" value="1" checked/>通过</label>&nbsp;&nbsp;
					    <label><input type="radio" name="check" value="2"/>未通过</label><br/>
					  </li>
					  
					  <li>
					    工作领域：<select id="field"><option value="">---请选择---</option>
					    <c:forEach items="${fields}" var="f">
					      <option value="${f.jfid}"><c:out value="${f.jfname}"></c:out></option>
					    </c:forEach>
					    </select> 
					    类别：<select id="class"><option value="">---请选择---</option></select> 
					    职位：<select id="job"><option value="">---请选择---</option></select><br/><br/>
					    行业：<select id="trade"><option value="">---请选择---</option>
					         <c:forEach items="${trades}" var="t">
					           <option value="${t.tid}"><c:out value="${t.tname}"></c:out></option>
					         </c:forEach></select>
					  <br/><br/>日期：<input type="date" id="jiReledate"/>
								...
								<table>
									<ul class="reset my_jobs">
										<li>
											...
											<div>
												<c:if test="${ujob.jiStatus eq 1}">
													审核状态：已通过
												</c:if>

												<c:if test="${ujob.jiStatus eq 2}">
													审核状态：未通过
												</c:if>
											</div>
											<div class="links">
												<a href="selectJaobInforAdmin?jobInfo.jiid=${ujob.jiid}"><input class="job" type="button" value="修改" /></a>
											</div>
										</li>
									</ul>
								</table>
