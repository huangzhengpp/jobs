          ...
          <dd>
			...
			<div class="content">
				...
				<dl class="company_center_content">
					...
					<li>
					<form id="searchForm" method="post" action="checkCominfo">
					<h3>高级查询</h3>
					<input type="hidden" name="status" value="1">
					<p>公司名称：<input type="text" name="comInfo.comName">
					-公司行业：
					<select name="trade.tid">
					<c:forEach items="${loginTrades }" var="t"><option value="${t.tid }">${t.tname }</option></c:forEach>
					...
					<c:forEach items="${comInfos}" var="c">
					<dd>
							<ul class="reset my_jobs">
								<li data-id="149594">
				<h3>ID:${c.cid }-${c.comName }
					</h3>
					
									
					<div class="c9">营业执照：${c.comPermitNum }</div>
				<div class="links">
				
					<a href="comdetail?comInfo.cid=${c.cid }&status=1">详情</a>
					...
					<c:if test="${empty comInfos}"><div style="width:200px;margin: auto;"><h2>查询无结果</h2></div><br></c:if>	
				<c:if test="${!empty comInfos}">
				
		<dd><ul class="reset"><li id="fenye">
		 <a href="checkCominfo?status=1&comInfo.comName=${comInfo.comName }&trade.tid=${trade.tid}">首页</a>
		 <a  href="checkCominfo?status=1&pageNo=${pageNo==1?1:pageNo-1 }&comInfo.comName=${comInfo.comName }&trade.tid=${trade.tid}">上一页</a>
		 <a href="checkCominfo?status=1&pageNo=${pageNo==pageNum?pageNum:pageNo+1 }&comInfo.comName=${comInfo.comName }&trade.tid=${trade.tid}">下一页</a>
		 <a href="checkCominfo?status=1&pageNo=${pageNum }&comInfo.comName=${comInfo.comName }&trade.tid=${trade.tid}">末页</a>
		 ${pageNo }/${pageNum}页</li>
		</ul>
