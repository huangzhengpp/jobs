    ...
    <div id="container">
        ...
        <c:if test="${loginSysuser.SType=='1' }">
          ...
          <dd>
			...
			<c:if test="${status eq 1}"><form action="updateFieldAdmin" method="post" name="form1"></c:if>
			<c:if test="${status eq 2}"><form action="updateClassAdmin" method="post" name="form2"></c:if>
		    <c:if test="${status eq 3}"><form action="updateJobAdmin" method="post" name="form3"></c:if>
				<dl class="company_center_content">
					<dt>
						<h1>
						<c:if test="${status eq 1}">修改工作领域</c:if>
				        <c:if test="${status eq 2}">修改工作类型</c:if>
				        <c:if test="${status eq 3}">修改工作职位</c:if>
						<span> </span>
						</h1>
					</dt><br/><br/>
					...
					<ul class="reset my_jobs">
					  ...
					  <li><br/>
					    ...
					    <c:if test="${status eq 1}">
					      <input type="hidden" value="${jobField.jfid}" name="jobField.jfid">
					      <input type="text" name="jobField.jfname" id="jobField" value="${jobField.jfname}">&nbsp;&nbsp;&nbsp;&nbsp;
					      <input type="button" value="修改" onclick="checkField()"><div id="msg"></div>
					    </c:if>
					    
					    <c:if test="${status ne 1}">
					    <select id="field" name="jobField.jfid">
					      <c:forEach items="${fields}" var="f">
					        <c:if test="${f.jfid eq jobField.jfid}">
					          <option value="${f.jfid}" selected="selected"><c:out value="${f.jfname}"></c:out></option>
					        </c:if>
					        
					        <c:if test="${f.jfid ne jobField.jfid}">
					...
					<ul class="reset my_jobs">
					  ...
					  <li><br/>
					    <c:if test="${status eq 2}">
					      <input type="hidden" value="${jobClass.jcid}" name="jobClass.jcid">
					      <input type="text" name="jobClass.jcname" id="jobClass" value="${jobClass.jcname}">&nbsp;&nbsp;&nbsp;&nbsp;
					      <input type="button" value="修改" onclick="checkClass()"><div id="msg"></div>
					    </c:if>
					    
					    <c:if test="${status ne 2}">
					    <select id="class" name="jobClass.jcid">
					      <c:forEach items="${jobClasses}" var="c">
					        <c:if test="${c.jcid eq jobClass.jcid}">
					          <option value="${c.jcid}" selected="selected"><c:out value="${c.jcname}"></c:out></option>
					        </c:if>
					        
					        <c:if test="${c.jcid ne jobClass.jcid}">
					...
					<ul class="reset my_jobs">
					  ...
					  <li><br/>
					    <input type="hidden" value="${jobName.jnid}" name="jobName.jnid">
					    <input type="text" name="jobName.jnname" id="jobName" value="${jobName.jnname}">&nbsp;&nbsp;&nbsp;&nbsp;
					    <input type="button" value="修改" onclick="checkJob()"><div id="msg"></div>
					  </li><li><br/></li>
					</ul>
					</dd><br/></c:if>
