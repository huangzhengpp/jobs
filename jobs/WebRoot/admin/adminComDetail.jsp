...
<![endif]-->
	<script type="text/javascript">
		...
		function update(cid,s){
			if(confirm("确认审核该账户吗？")){
					window.location.href="checkOperAndCom?comInfo.cid="+cid+"&status="+s;
          ...
          <dd>
			...
			<div class="content">
				...
				<dl class="company_center_content">
					<dt>
						<h1>
							${comInfo.comStatus=='0'?'待审核企业':'已通过审核企业'}
							<span> </span>
						</h1>
					</dt> 
					<dd><ul id="gaoji" class="reset my_jobs">
					<li>
					<p>公司登录账户ID：${comInfo.oper.oid }</p>
					<p>公司信息状态：${comInfo.comStatus=='0'?'待审核企业':'已通过审核企业'}</p>
					<p>公司名称：${comInfo.comName }</p>
					<p>公司地址：${comInfo.comAddr }</p>
					<p>公司性质：${comInfo.comPro }</p>
					<p>公司规模：${comInfo.comScale }</p>
					<p>公司介绍：${comInfo.comInfo }</p>
					<p>公司座机：${comInfo.comPhone }</p>
					<p>联系电话：${cominfo.comMobilephone }</p>
					<p>公司邮箱：${comInfo.comEmail }</p>
					<p>联系人：${comInfo.comContact }</p>
					<p>营业执照编号：${comInfo.comPermitNum }</p>
					<p>营业执照文件：
					<img alt="营业执照" src="images/${comInfo.comPermitFile }">
					</p>
					<div style="width:250px;margin: auto;">
					<p>
						<c:if test="${comInfo.comStatus=='0'}"><a href="javascript:update(${comInfo.cid },1)">审核通过</a>--<a href="javascript:update(${comInfo.cid },2)">审核不通过</a></c:if>
					<c:if test="${comInfo.comStatus=='1'}"><a href="javascript:update(${comInfo.cid },2)">撤销审核</a></c:if>
					</p> 
